...
~~**Normalize the wardrobe storage folder convention.**~~ **Done & deployed (2026-07-12).** Studio components take an explicit `{ wardrobeId, ownerId }` pair; uploads route through `lib/wardrobe-paths.ts` (owner folder `<user_id>/…`, ownerless intake under `wardrobe/<id>/…`); the `user_id: clientId` insert corruption is gone. Migration 07 replaced migration 03's authenticated-only policy with owner-or-admin via `public.can_access_wardrobe_object(...)`; migrations 06 + 07 applied to prod, `scripts/verify_wardrobe_policy.ts` passes 6/6. Spec/plan under `docs/superpowers/{specs,plans}/2026-07-11-wardrobe-storage-normalization*`.
