...
- **Paid content**: `chapters.lab_questions` / `resource_urls` and `masterclasses.resource_urls` are not readable by browser roles (migration 30). Read them with `app/lib/paid-content.ts` after `checkAccess()`; select catalogue rows with `CHAPTER_CATALOG_COLUMNS` / `MASTERCLASS_CATALOG_COLUMNS` (`app/lib/chapter-columns.ts`), never `*`. Paid downloads live in the private `vault-resources` bucket as `{ name, path }`.
